...
`ibm.stickleback` and written to the working directory 
...
All these objects are accessed with `$`, see the next step.

## Visualization

1. To view the IBM-PCoA plot:
```r
...
2. Create a new object, outside the list, for the heatmap figure:
```r
...
4. View the table with summary of missing genotypes per individuals: 
```r
table.ind <- ibm.stickleback$missing.genotypes.ind
```
...
5. To view the distribution of missing genotypes per individuals
```r
ibm.stickleback$missing.genotypes.ind.plot
```

6. To view the distribution of missing genotypes per markers and populations
```r
...
7. Explore the rest by yourself!
...
* explore the impact of different filtering parameters on missing genotypes, with available arguments: e.g. `snp.ld`, `common.markers`, `maf.thresholds` arguments to quickly generate the figures.
