...
We will keep the first SNP on every read. 
    * Minor Allele Frequency filter: for ∂a∂i we want to keep most of the 
Allele Frequency Spectrum, so we are filtering differently than for population 
assignment. Here we want to make sure we don't include the SNP with very 
very low frequency, the uninformative markers. So we ask to remove markers if the 
...
1. To view the content of `dadi.df`:
```r
names(dadi.df)
```
