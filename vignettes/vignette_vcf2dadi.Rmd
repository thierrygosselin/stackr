...
have the required column names  (`INDIVIDUALS` and `STRATA`). 
  ...
  * We could put all the lines below in one call, but for clarity I'll break down
the steps:
```r
...
```
    ...
    72 sec later, this is what I get in my RStudio console:
    ```r
    #######################################################################
########################## stackr::vcf2dadi ###########################
#######################################################################
...
#######################################################################
    ```
1. Since I didn't redirect the function call in an object 
...
It make no sense to create a ∂a∂i input file with so many populations. If we
wanted to keep just 3 populations: Had, Mos and Odd, we could use the `pop.select` 
argument. This is where the default argument `common.markers = TRUE` and 
`monomorphic.out = TRUE` becomes useful. Both can be turned off with `FALSE`. 

Monomorphic sites are useful to get absolute numbers for the timing event. 
Laurent Excoffier says that *it allows one to use the mutation rate to calibrate 
the parameters on an absolute scale* (see this [thread](https://groups.google.com/forum/#!searchin/fastsimcoal/impact$20of$20monomorphic/fastsimcoal/dkJ6P5y1ViY/Jn977V8EAwAJ), for more info on the utility of monomorphic sites and SFS).

However, here the RAD markers were filtered based on 8 populations, 
consequently, to avoid filtering bias, it's better to remove the 
monomorphic markers and markers not in common between the 3 populations.
...
least 1 population (out of 3 the toal selected). For further details on how 
...
dadi.df <- stackr::vcf2dadi(
  ...
  #here I specify a custom output filename
  dadi.input.filename = "my.first.dadi.input.file.tsv"
)
```
    60 sec later, this is what I get in my RStudio console:
    ```r
    #######################################################################
########################## stackr::vcf2dadi ###########################
#######################################################################
