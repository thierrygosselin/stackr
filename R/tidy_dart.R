...
tidy_dart <- function(
  ...
  input <- suppressWarnings(
    input %>% 
      ...
      dplyr::mutate(MARKERS = stringi::stri_join(LOCUS, POS, sep = "_"))
  )
  ...
  if (binary == 2) {
    ...
    grouping.column <- input %>% 
      ungroup() %>% 
      dplyr::select(MARKERS, LOCUS, POS, REF, ALT, CALL_RATE, AVG_COUNT_REF,
                    AVG_COUNT_SNP, REP_AVG) %>% 
      dplyr::filter(!is.na(REF) | !is.na(ALT)) %>% 
      dplyr::distinct(MARKERS, LOCUS, POS, REF, ALT, CALL_RATE, AVG_COUNT_REF,
    ...
    # Data tidying
    input <- input %>% 
      dplyr::select(-c(LOCUS, POS, REF, ALT, CALL_RATE, AVG_COUNT_REF,
                       AVG_COUNT_SNP, REP_AVG)) %>% 
      dplyr::mutate(
        ALLELE_NAME = rep("A", n()),
      ...
      dplyr::select(MARKERS, LOCUS, POS, REF, ALT, INDIVIDUALS, GT, CALL_RATE,
  ...
  if (!is.null(filter.reproducibility)) {
    ...
    whitelist.filter <- filter %>% dplyr::select(MARKERS, LOCUS, POS) %>% dplyr::distinct(MARKERS, LOCUS, POS)
    ...
    if (length(blacklist.markers.reproducibility$MARKERS > 0)) {
      readr::write_tsv(x = blacklist.markers.reproducibility, path = "blacklist.markers.reproducibility.tsv", col_names = TRUE)
    }
    if (plot.reproducibility) {
      ...
      plot.repro <- ggplot2::ggplot(data.combined, ggplot2::aes(x = factor(POP_ID), y = REP_AVG, na.rm = TRUE))+
  ...
  if (!is.null(filter.coverage.high) | !is.null(filter.coverage.low) & (plot.coverage)) {
    ...
    plot.cov <- ggplot2::ggplot(data.combined, ggplot2::aes(x = factor(POP_ID), y = AVG_COUNT_SNP, na.rm = TRUE))+
      ggplot2::geom_violin(trim = TRUE)+
      ggplot2::geom_boxplot(width = 0.1, fill = "black", outlier.colour = NA)+
      ggplot2::stat_summary(fun.y = "mean", geom = "point", shape = 21, size = 2.5, fill = "white")+
      ggplot2::labs(x = "Sampling sites")+
  ...
  input.before.filter <- plot.cov <- NULL # no longer needed
  ...
  if (!is.null(filter.call.rate)) {
    ...
    if (plot.call.rate) {
      ...
      plot.cr <- ggplot2::ggplot(data.combined, ggplot2::aes(x = factor(POP_ID), y = CALL_RATE, na.rm = TRUE))+
        ggplot2::geom_violin(trim = TRUE)+
        ggplot2::geom_boxplot(width = 0.1, fill = "black", outlier.colour = NA)+
        ggplot2::stat_summary(fun.y = "mean", geom = "point", shape = 21, size = 2.5, fill = "white")+
        ggplot2::labs(x = "Sampling sites")+
  ...
  if (!is.null(filter.ind.missing.geno)) {
    ...
    if (plot.ind.missing.geno) {
      ...
      ind.missing.geno.plot <- ggplot2::ggplot(data.combined, ggplot2::aes(x = factor(POP_ID), y = GENOTYPED_PROP, na.rm = TRUE))+
        ggplot2::geom_violin(trim = TRUE)+
        ggplot2::geom_boxplot(width = 0.1, fill = "black", outlier.colour = NA)+
        ggplot2::stat_summary(fun.y = "mean", geom = "point", shape = 21, size = 2.5, fill = "white")+
        ggplot2::labs(x = "Sampling sites")+
  ...
  if (!is.null(filter.markers.missing.ind)) {
    ...
    if (plot.markers.missing.ind) {
      ...
      markers.missing.ind.plot <- ggplot2::ggplot(data.combined, ggplot2::aes(x = factor(POP_ID), y = MISSING_IND_PROP, na.rm = TRUE))+
        ggplot2::geom_violin(trim = TRUE)+
        ggplot2::geom_boxplot(width = 0.1, fill = "black", outlier.colour = NA)+
        ggplot2::stat_summary(fun.y = "mean", geom = "point", shape = 21, size = 2.5, fill = "white")+
        ggplot2::labs(x = "Sampling sites")+
  ...
  if (plot.number.snp.reads) {
    ...
    number.snp.reads.plot <- ggplot2::ggplot(number.snp.reads, ggplot2::aes(factor(SNP_N)))+
      ggplot2::geom_bar()+
  ...
  # Whitelist 
  res$whitelist.markers <- dplyr::distinct(input, MARKERS, LOCUS, POS)
  ...
  # filtered tidy data
  res$tidy.data <- input %>% 
    dplyr::mutate(CHROM = rep("CHROM_1", n())) %>% 
