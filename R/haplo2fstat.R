...
#' @references Goudet, J. (2005) hierfstat, a package for r to compute and 
#' test hierarchical F‐statistics. Molecular Ecology Notes, 5, 184–186.
...
haplo2fstat <- function(haplotypes.file, 
                        whitelist.loci = NULL, 
                        blacklist.id = NULL, 
  ...
  nu.message <- stri_paste("The highest number used to label an allele, nu = ", nu, sep = "")
  message(nu.message)
  ...
  write_delim(x = haplo.prep, path = fstat.filename, delim = "\t", append = TRUE, col_names = FALSE)
  ...
  if (imputations != "FALSE"){
    ...
    nu.message <- stri_paste("With imputed data, the highest number used to label an allele, nu = ", nu, sep = "")
    message(nu.message)
    ...
    fstat.filename.imp <- stri_replace_all_fixed(fstat.filename,
                                                 pattern = ".dat",
    ...
    # FSTAT: write the pop and genotypes
    write_delim(x = haplo.imp, path = fstat.filename.imp, delim = "\t", append = TRUE, col_names = FALSE)
    
    invisible(cat(sprintf("fstat (with imputation) file name:\n%s\n\nWritten in the directory:\n%s", fstat.filename.imp, getwd())))
  }
}
