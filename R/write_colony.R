...
write_colony <- function(
  ...
  if (is.null(filename)) {
    ...
    if (!is.null(imputation.method)) {
      filename.imp <- stringi::stri_replace_all_fixed(
        str = filename,
        pattern = "stackr_colony_",
        replacement = "stackr_colony_imputed_",
        vectorize_all = FALSE
      )
    }
  } else {
  ...
  stackr_colony(data = input, filename = filename)
  ...
  if (!is.null(imputation.method)) {
    ...
    stackr_colony(data = input.imp, filename = filename.imp)
...
# stackr_colony internal function ----------------------------------------------
# @keywords internal
