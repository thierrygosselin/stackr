# Imputation of haplotype using missForest
...
haplo_imputation <- function(haplotypes.file, 
                                 whitelist.loci = NULL, 
                                 blacklist.id = NULL, 
...
#       NUCLEOTIDES = str_pad(NUCLEOTIDES, 3, side = "left", pad = "0"),
      NUCLEOTIDES = stri_replace_all_fixed(NUCLEOTIDES, "0", "NA", vectorize_all=F)
    ) %>%
    select(-Cnt) %>%
    dcast(INDIVIDUALS ~ Catalog.ID + ALLELE, value.var = "NUCLEOTIDES")
  
  
  # Create an optional column with the population (strata) ordered by levels
...
# res <- gtypes(gen.data = gtypes, id.col = "INDIVIDUALS", strata.col = "POP_ID", locus.col = 3, dna.seq = NULL, description = description)



#   if (missing(filename) == "FALSE") {
#     message("Saving the file in your working directory...")
#     write_tsv(gtypes, filename, append = FALSE, col_names = TRUE)
