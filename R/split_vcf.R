...
#' @param strata A file identical to the strata file usually used in stackr,
...
split_vcf <- function(
  ...
  filename <- stringi::stri_join("stackr_split_vcf_", file.date)
  ...
  split_vcf <- function(data, filename) {
    ...
    stackr::write_vcf(data = dplyr::select(data, -SPLIT),
