#' @title Combination of all filters with a tidy vcf
#' @description under construction...
#' @param vcf.tidy Working on it.
...
#' @param filename Name of the whitelist file written to the directory.
#' @return A list with 3 dataframes (whitelist, blacklist.genotypes, tidy.vcf.filtered)
#' @rdname filter_quick
#' @export
#' @import stringi
#' @import dplyr
...
filter_quick <- function (vcf.tidy,
  ...
  whitelist <- all.filters %>%
    select(LOCUS) %>%
    distinct(LOCUS) %>%
    arrange(LOCUS)
  
  vcf.tidy.filtered <- whitelist %>%
    left_join(vcf.erased, by = "LOCUS") %>% 
  ...
  res$vcf.tidy.filtered <- vcf.tidy.filtered
  
  if (missing(filename) == "FALSE") {
    message("Saving the file in your working directory...")
    write_tsv(whitelist, filename, append = FALSE, col_names = TRUE)
