...
# Individual
individual_sumstats_filter <- function(data, population.map, pop.id.start, pop.id.end, pop.levels, ind.threshold, threshold.fixed ) {
...
invisible(
        cat(
         sprintf(
"Individual filter: %s percent or proportion of genotyped individuals per sampling sites to keep the marker
The number of LOCI before the individual filter = %s LOCI
The number of LOCI removed by the individual filter = %s LOCI
The number of SNP after the individual filter = %s SNP
The number of LOCI after the individual filter = %s LOCI", 
ind.threshold,
n_distinct(stacks.vcf.file$LOCUS),
n_distinct(stacks.vcf.file$LOCUS)-n_distinct(ind.filter$LOCUS),
n_distinct(ind.filter$POS),
n_distinct(ind.filter$LOCUS)
        )
        )
...
# Population
population_filter <- function(data, pop.threshold, percent) {
  ...
  "Population filter: %s sampling sites/pop required to keep the marker
...
population_vcf_filter <- function(data, pop.threshold, percent) {
