...
write_vcf <- function(data, pop.info = FALSE, filename = NULL) {
  ...
  # File format ----------------------------------------------------------------
  readr::write_delim(x = data_frame("##fileformat=VCFv4.3"), path = filename, delim = " ", append = FALSE, col_names = FALSE)
  
  # File date ------------------------------------------------------------------
  file.date <- stringi::stri_replace_all_fixed(Sys.Date(), pattern = "-", replacement = "")
  file.date <- stringi::stri_join("##fileDate=", file.date, sep = "")
  readr::write_delim(x = data_frame(file.date), path = filename, delim = " ", append = TRUE, col_names = FALSE)
