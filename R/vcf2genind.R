...
vcf2genind <- function(vcf.file, 
  ...
  if (is.null(whitelist.markers) | missing(whitelist.markers)) {
  ...
  vcf <- suppressWarnings(
    vcf %>%
      ...
      tidyr::gather(INDIVIDUALS, FORMAT_ID, -MARKERS) %>% 
      mutate(
  ...
  vcf <- vcf %>% 
    mutate(
      GT = ifelse(GT == "0/0", "111/111",
                  ifelse(GT == "1/1", "222/222",
                         ifelse(GT == "0/1", "111/222",
                                ifelse(GT == "1/0", "222/111", "000/000"))))
    ) %>% 
  ...
  res <- adegenet::df2genind(X = genind.df, sep = "/", ind.names = ind, pop = pop, ploidy = 2, strata = strata, hierarchy = NULL)
  ...
  if (imputations != "FALSE"){
    ...
    res$imputed <- adegenet::df2genind(X = genind.df, sep = "/", ind.names = ind, pop = pop, ploidy = 2, strata = strata, hierarchy = NULL)
